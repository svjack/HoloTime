...
def load_data_prompts(data_dir, guidance_dir, guidance_suffix, video_size=(256,256), video_frames=16, interp=False, device_num=0):
    ...
    for idx in range(n_samples):
        if interp:
            ...
            image1 = transform0(image1)
            image1 = torch.cat((image1, image1[:, :, :image1.size(-1)//15]), dim=2)
            image_tensor1 = transform(image1).unsqueeze(1) # [c,1,h,w]
            image2 = Image.open(file_list[2*idx+1]).convert('RGB')
            image2 = transform0(image2)
            image2 = torch.cat((image2, image2[:, :, :image2.size(-1)//15]), dim=2)
            ...
            image = torch.cat((image, image[:, :, :image.size(-1)//15]), dim=2)
            image_tensor = transform(image).unsqueeze(1) # [c,1,h,w]
            frame_tensor = repeat(image_tensor, 'c t h w -> c (repeat t) h w', repeat=video_frames)
            _, filename = os.path.split(file_list[idx])

        #c, t, h, w = frame_tensor.shape
        data_list.append(frame_tensor)
        filename_list.append(filename)

        if guidance_dir is not None:
            # Read Guidance Video
            video_path = os.path.join(guidance_dir, filename.split('.')[0], filename.split('.')[0]+f'_{guidance_suffix}.mp4')            
            video_reader = VideoReader(video_path, ctx=cpu(0))
            video = video_reader.get_batch(list(range(video_frames))).asnumpy()

            print('Super resolution for guidance video')
            video = realesrgan_x2(video, device=f'cuda:{device_num}')

            video = torch.tensor(np.array(video)).permute(0, 3, 1, 2).float()/255.0  # [t,h,w,c] -> [t,c,h,w]
            video = torch.cat((video, video[...,:video.size(-1)//15]), dim=-1)
            video = transform(video).permute(1, 0, 2, 3)#.unsqueeze(0) # [t,c,h,w] -> [c,t,h,w]
            
            video_list.append(video)

    return filename_list, data_list, prompt_list, video_list
...
def save_results_seperate(prompt, samples, filename, savedir, save_suffix, fps=10, loop=False):
    prompt = prompt[0] if isinstance(prompt, list) else prompt
    ...
    for idx, video in enumerate(videos):
        ...
        for i in range(n):
            ...
            if n == 1:
                i = ''
            os.makedirs(os.path.join(savedir, filename.split(".")[0]), exist_ok=True)
            path = os.path.join(savedir, filename.split(".")[0], f'{filename.split(".")[0]}_{save_suffix}{i}.mp4')
...
def image_guided_synthesis(model, prompts, images, video_inputs, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    if video_inputs is not None:
        x_T = get_latent_z(model, video_inputs)
    else:
        x_T = None
    ...
    for _ in range(n_samples):
        ...
        model.restoreConv2D3DMethods()
        batch_images = ddim_sampler.blend_h(batch_images, batch_images, batch_images.shape[-1]//16, True, True) 
        
        # Crop repeated part
        width = batch_images.shape[-1]
        height = 512 if batch_images.shape[-2] > 512 else batch_images.shape[-2]
        batch_images = batch_images[:,:,:,:,:-batch_images.shape[-1]//16].squeeze(0)
        batch_images = F.interpolate(batch_images, size=(height, width), mode='bilinear').unsqueeze(0)
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            for nn, samples in enumerate(batch_samples):
                ...
                save_results_seperate(prompt, samples, filename, args.savedir, args.save_suffix, fps=args.frame_stride, loop=args.loop)
...
def get_parser():
    ...
    parser.add_argument("--save_suffix", type=str, default="refinement", help="guidance video suffix")
    parser.add_argument("--ckpt_path", type=str, default=None, help="checkpoint path")
    parser.add_argument("--config", type=str, help="config (yaml) path")
    parser.add_argument("--prompt_dir", type=str, default=None, help="a data dir containing images and prompts")
    parser.add_argument("--guidance_dir", type=str, default=None, help="a data dir containing guidance videos")
    parser.add_argument("--guidance_suffix", type=str, default="guide", help="guidance video suffix")
...
if __name__ == '__main__':
    ...
    if args.guidance_dir:
        print("#-------------------------Panoramic Animator Refinement Inference: %s -------------------------#"%now)
    else:
