...
def preprocess(input_frames, extend):
    out_frame_list = []
    ...
    # copy and concat
    out_frames = torch.cat((out_frames, out_frames[...,:extend]), dim=-1)
...
def save_video(video, save_dir, file_name, fps=16.0):
    ...
    if isinstance(video[0], torch.Tensor):
        ...
        images = [frame for frame in video]
    else:
...
def blend_h(a, b, blend_extent, left, right):
