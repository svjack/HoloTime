...
from image_warper.warper_method_gpu import Warper
...
def gt_warping(rgb_gt, depth_gt, pose_gt, poses_tar, H, W, intrinsic, logpath=None, mask_gt=None, warp_depth=False, bilinear_splat=False):
    ...
    if bilinear_splat:
        warper = Warper()
        transformation1 = torch.linalg.inv(pose_gt).float().to(pose_gt.device)
        intrinsic_mtx = torch.eye(3, device=pose_gt.device).float()
        intrinsic_mtx[0, 0] = intrinsic[0]
        intrinsic_mtx[1, 1] = intrinsic[1]
        intrinsic_mtx[0, 2] = intrinsic[2]
        intrinsic_mtx[1, 2] = intrinsic[3]
        #rgb_src = rgb_gt.astype(np.uint8)
        # rgb_src 为torch
        rgb_src = rgb_gt.cuda()
        #poses_np = poses_tar.cuda()

        #rgb_src = torch.from_numpy(rgb_src).cuda()
        depth_gt = depth_gt.cuda().float()
        #transformation1 = torch.from_numpy(transformation1).cuda().float()
        #intrinsic_mtx = torch.from_numpy(intrinsic_mtx).cuda()

        for vv in range(poses_tar.shape[0]):
            transformation2 = torch.linalg.inv(poses_tar[vv]).float().to(pose_gt.device)
            #transformation2 = torch.from_numpy(transformation2).cuda().float()
            ...
            warped_frame2 = warped_frame2.cpu().numpy()
            mask2 = mask2.cpu().numpy()
            warped_depth2 = warped_depth2.cpu().numpy()
            flow12 = flow12.cpu().numpy()
            # obtain white background
            for i in range(3):
                warped_frame2[:,:,i] = np.array(warped_frame2[:,:,i]*mask2+255*(1-mask2))
            rgbs_warp.append(warped_frame2)
