...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if adapter_config is not None:
            ...
            # freeze unet, only train the adapter
            self.model.eval()
    ...
    def apply_model(self, x_noisy, t, cond, **kwargs):
        ...
        if self.adapter is not None:
            ...
            x_copy = rearrange(x_copy, "b c f h w -> (b f) c h w")
            features_adapter = self.adapter(x_copy)
        ...
        x_recon = self.model(x_noisy, t, features_adapter=features_adapter, **cond, **kwargs)
...
class LatentVisualDiffusion(LatentDiffusion):
    def __init__(self, img_cond_stage_config, image_proj_stage_config, freeze_embedder=True, image_proj_model_trainable=True, freeze_temporal_block=False, *args, **kwargs):
        ...
        freeze_list = []
        if freeze_temporal_block:
            for name, module in self.model.diffusion_model.named_modules():
                module_type_name = type(module).__name__
                if module_type_name == 'TemporalTransformer' or module_type_name == 'TemporalConvBlock':
    ...
    def circularConv2D3DMethods(
        self, tileX: bool, tileY: bool, tileZ: bool, startStep: int, stopStep: int, step: int
    ):
        modules_all = [item for item in self.model.named_modules()] + [item for item in self.first_stage_model.named_modules()]
        for name, module in modules_all:
            if isinstance(module, torch.nn.Conv2d):
                ...
                module._conv_forward = (
                    LatentVisualDiffusion.__replacementConv2DConvForward.__get__(
            ...
            if isinstance(module, torch.nn.Conv3d):
                module.padding_modeX = "circular" if tileX else "constant"
                module.padding_modeY = "circular" if tileY else "constant"
                module.padding_modeZ = "circular" if tileZ else "constant"
                module.paddingX = (
                    module._reversed_padding_repeated_twice[0],
                    module._reversed_padding_repeated_twice[1],
                    0,
                ...
                module.paddingZ = (
                    ...
                    0,
                    0,
                    module._reversed_padding_repeated_twice[4],
                    module._reversed_padding_repeated_twice[5],
                )
                module.paddingStartStep = startStep
                module.paddingStopStep = stopStep
                module.paddingStep = step
                module._conv_forward = (
                    LatentVisualDiffusion.__replacementConv3DConvForward.__get__(
    ...
    def __replacementConv2DConvForward(
        ...
        if (
            ...
            if self.paddingX[0] > 0:
                left = input[..., -self.paddingX[0]-w:-w]
                input2 = torch.cat([left, input], dim=-1)
            else:
                input2 = input
            if self.paddingX[1] > 0:
                right = input[..., w:self.paddingX[1]+w]
                working = torch.cat([input2, right], dim=-1)
            else:
                working = input2
    ...
    def __replacementConv3DConvForward(
        self, input: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor]
    ):
        if (
            self.paddingStartStep < 0 or self.paddingStep >= self.paddingStartStep
            ...
            if self.paddingX[1] > 0:
                ...
                working = input2
            working = F.pad(working, self.paddingY, mode=self.padding_modeY)
            working = F.pad(working, self.paddingZ, mode=self.padding_modeZ)
        else:
            working = F.pad(input, self.paddingX, mode="constant")
            working = F.pad(working, self.paddingY, mode="constant")
            working = F.pad(working, self.paddingZ, mode="constant")
        return F.conv3d(
            working, weight, bias, self.stride, (0, 0, 0), self.dilation, self.groups
        )

    def restoreConv2D3DMethods(self):
        modules_all = [item for item in self.model.named_modules()] + [item for item in self.first_stage_model.named_modules()]
        for name, module in modules_all:
            if isinstance(module, torch.nn.Conv2d):
                ...
                module._conv_forward = torch.nn.Conv3d._conv_forward.__get__(
                    module, torch.nn.Conv3d
                )
