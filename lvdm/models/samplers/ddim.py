...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True, X_T=None):
        self.ddim_timesteps = make_ddim_timesteps(ddim_discr_method=ddim_discretize, num_ddim_timesteps=ddim_num_steps,
                                                  num_ddpm_timesteps=self.ddpm_num_timesteps,verbose=verbose)
        
        if X_T is not None:
            self.ddim_timesteps = self.ddim_timesteps[:len(self.ddim_timesteps)//10*8] # time step for guidance
            self.model.alphas_cumprod = self.model.alphas_cumprod[:self.ddim_timesteps[-1]+1]
            self.model.alphas_cumprod_prev = self.model.alphas_cumprod_prev[:self.ddim_timesteps[-1]+1]
            self.model.betas = self.model.betas[:len(self.ddim_timesteps)]
    ...
    def sample(self,
        ...
        self.make_schedule(ddim_num_steps=S, ddim_discretize=timestep_spacing, ddim_eta=eta, verbose=schedule_verbose, X_T=x_T)
        ...
        if x_T is not None:
            ...
            print(f"Guidance timesteps: {self.ddim_timesteps[-1]} / {self.ddpm_num_timesteps}")
            x_T = x_T * self.model.sqrt_alphas_cumprod[self.ddim_timesteps[-1]] + e * self.model.sqrt_one_minus_alphas_cumprod[self.ddim_timesteps[-1]]
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      fs=None,guidance_rescale=0.0,blend_latent=True,img=None,
                      **kwargs):
        ...
        left = True
        right = False
        circular_padding = True if img.size(-1) > 100 else False
        ...
        for i, step in enumerate(iterator):
            ...
            if circular_padding:
                self.model.circularConv2D3DMethods(
                    True, False, False, 0, len(iterator), i
                )
            ...
            # blend repeat latents
            if blend_latent:
                blend_extent = img.shape[-1] // 16
                img = self.blend_h(img, img, blend_extent, left, right) 
                left = not left
                right = not right
        
        return img, intermediates

    def blend_h(self, a, b, blend_extent, left, right):
        blend_extent = min(a.shape[4], b.shape[4], blend_extent)
        for x in range(blend_extent):
            if left:
                b[:, :, :, :, x] = a[:, :, :, :, -blend_extent
                              + x] * (1 - x / blend_extent) + a[:, :, :, :, x] * (
                                  x / blend_extent)
            if right:
                b[:, :, :, :, -blend_extent + x] = a[:, :, :, :, -blend_extent
                              + x] * (1 - x / blend_extent) + a[:, :, :, :, x] * (
                                  x / blend_extent)
        return b
