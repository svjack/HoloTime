...
class ResnetBlock_pseudo3d(nn.Module):
    def __init__(
        self, in_c, out_c, down, ksize=3, sk=False, use_conv=True, frame_num=25
    ):
        super().__init__()
        ps = ksize // 2
        self.down = down
    ...
    def forward(self, x):
        ...
        if self.in_conv is not None:
            x1 = self.in_conv(x)
        print(x1.shape, '!!')
        x1 = rearrange(x1, "(b f) c h w -> b c f h w", f=self.frame_num)
        h = self.block1(x1)
        print(h.shape, '!!!')
        h = self.act(h)
        print(h.shape, '!!!!')
        ...
        if self.skep is not None:
            skip_x = self.skep(x)
            h = h + rearrange(skip_x, "(b f) c h w -> b c f h w", f=self.frame_num)
        else:
            h = h + x1
