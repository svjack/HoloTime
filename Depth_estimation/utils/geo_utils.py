...
def panorama_to_pers_cameras(ratio=1.):
    ...
    down_vecs = torch.cat([-down_vecs[:, 1:2], -down_vecs[:, 2:], down_vecs[:, :1]], dim=1)
    right_vecs = torch.cat([-right_vecs[:, 1:2], -right_vecs[:, 2:], right_vecs[:, :1]], dim=1)
    to_vecs = torch.cat([-to_vecs[:, 1:2], -to_vecs[:, 2:], to_vecs[:, :1]], dim=1)
    #print(down_vecs.shape)
